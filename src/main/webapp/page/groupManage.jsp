...
<div id="tb">
	<div>
	<a href="javascript:;" onclick="openGroupAddDiglog()" class="easyui-linkbutton" iconCls="icon-add" plain="true">添加</a>
	<a href="javascript:;" onclick="openGroupModifyDiglog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a>
	<a href="javascript:;" onclick="deleteGroup()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
 	</div>
</div>
...
<div id="dlg-buttons">
	<a href="javascript:;" onclick="checkData()" class="easyui-linkbutton" iconCls="icon-ok">保存</a>
	<a href="javascript:;" onclick="closeGroupDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
</div>
...
function checkData(){
	...
	if(flag==1){
		$.post("${pageContext.request.contextPath}/group/existGroupName.do",{groupName:$("#id").val()},function(result){
			if(result.exist){
				$.messager.alert("系统提示","该角色名已存在，请更换！");
				$("#id").focus();
			}else{
				saveGroup();
			}
