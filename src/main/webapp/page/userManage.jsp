...
<div id="tb">
	<div>
	<a href="javascript:;" onclick="openUserAddDiglog()" class="easyui-linkbutton" iconCls="icon-add" plain="true">添加</a>
	<a href="javascript:;" onclick="openUserModifyDiglog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a>
	<a href="javascript:;" onclick="deleteUser()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
 	</div>
 	<div>
...
function checkData(){
	...
	if(flag==1){
		$.post("${pageContext.request.contextPath}/user/existUserName.do",{userName:$("#id").val()},function(result){
			if(result.exist){
				$.messager.alert("系统提示","该用户名已存在，请更换！");
				$("#id").focus();
			}else{
				saveUser();
			}
